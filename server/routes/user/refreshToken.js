...
router.post('/', async (req, res, next) => {
    ...
    try {
        ...
        if(payload.exp * 1000 >= Date.now()) {
            ...
            // If not exits
            if(index === -1) throw Error('Your account may compromised. Please login again. And also remove your session');

            if(!user.sessions[index].verified) throw Error('Please Conferm your session with the email address');
